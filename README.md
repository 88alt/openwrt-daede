...
1. **性能优化栈**（相比原版 dae 的核心差异）
   ...
   - **QUIC**：基线 + 我们自持的性能补丁（`ci/patches/quic-go/`，B-tree 节点池等），复现的 perf tip **正是官方 `daeuniverse/dae` go.mod 自己 pin 的那个**（其 go.mod 注释原话：*"Use optimized quic-go with B-tree node pooling + upstream cherry-picks"*）——不是我们随便挑的 fork，而是官方背书的优化版；所以和官方完全同步，不依赖 olicesx 分支存活（`auto-bump` 的告警也改成只在官方 dae 换 quic-go 时才提醒）
